...
try {
    ...
    if (!isset($input['seller_id']) || !is_numeric($input['seller_id'])) {
        throw new Exception("Seller ID is required and must be numeric");
    }
    ...
    $changedBy = $input['changed_by'] ?? "seller_$sellerId";
    $notes = $input['notes'] ?? '';

    // Validate status using shared function
    if (!isValidOrderStatus($newStatus)) {
        throw new Exception("Invalid status. Must be one of: pending, confirmed, processing, shipped, delivered, cancelled, refunded");
    }
    ...
    if (!canAccessOrder($connection, $orderId, $sellerId, 'seller')) {
        throw new Exception("Access denied - Order not found or does not belong to seller");
    }

    // Check if order exists and get current status
    $checkSql = "SELECT id, status FROM orders WHERE id = $orderId AND seller_id = $sellerId";
